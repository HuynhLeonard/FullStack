...
export const loginUser = async (req,res,next) => {
    try {
        ...
        if(await user.comparePassword(password)) {
            ...
            throw new Error("Email Invalid or password!");
        }
    } catch (err) {
        next(err);
    }
...
export const updatedProfile = async (req,res,next) => {
    try {
        ...
        // Nếu không cập nhật thì chọn lại cái sẵn có.
        // hoặc có thể dùng find by id and update with $set
        user.name = req.body.name || user.name;
        user.email = req.body.email || user.email;
        if(req.body.password && req.body.password.length < 5) {
            throw new Error("Password length must be at lease 5 letters");
